...
from DomainBuilder.pipeline import DomainPipelineConfig, run_domain_pipeline
...
pipeline_config = DomainPipelineConfig()


def main() -> None:
    for sample_id in sample_ids:
        program_bundle = dataset.sample_dir(sample_id) / "program_bundle"
        out_root = dataset.out_root()
        print(f"Processing {dataset.dataset_id} / {sample_id} ...")
        domain_bundle = run_domain_pipeline(
            program_bundle_path=program_bundle,
            out_root=out_root,
            sample_id=sample_id,
            config=pipeline_config,
        )
        print(f"Done: {domain_bundle}")
